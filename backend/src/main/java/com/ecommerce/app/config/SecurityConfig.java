...
public class SecurityConfig {
	...
	private String allowedOrigins; // e.g. http://localhost:5173,http://localhost:3000
	...
	public SecurityFilterChain filterChain(
		...
		corsConfig.setAllowedOriginPatterns(Arrays.asList(allowedOrigins.split(","))); // ✅ FIXED
		corsConfig.setAllowedMethods(Arrays.asList("GET", "POST", "PUT", "DELETE", "OPTIONS"));
		corsConfig.setAllowedHeaders(Arrays.asList("*"));
		corsConfig.setAllowCredentials(true);

		http.cors(cors -> cors.configurationSource(r -> corsConfig)).csrf(csrf -> csrf.disable())
